...
function ModalCreer({ onClose, onSuccess }) {
  const [form, setForm] = useState({ nom: '', email: '', password: '', role: 'employe', brigade: '' });
      ...
      <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="form-grid">
          <div className="form-group full">
            ...
            <input type="text" value={form.nom} onChange={e => setForm(p => ({ ...p, nom: e.target.value }))} placeholder="Ex: Mohamed Ahmed" />
          </div>
          <div className="form-group full">
            <label>Email *</label>
            <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="email@snde.mr" />
          </div>
          <div className="form-group full">
            <label>Mot de passe *</label>
            <input type="password" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} placeholder="Minimum 6 caracteres" />
          </div>
...
function ModalModifier({ user, onClose, onSuccess }) {
  const [form, setForm] = useState({ role: user.role || 'employe', brigade: user.brigade || '', actif: user.actif !== false });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="form-grid">
          ...
          <div className="form-group">
            <label>Statut</label>
            <select value={form.actif ? 'actif' : 'inactif'} onChange={e => setForm(p => ({ ...p, actif: e.target.value === 'actif' }))}>
...
with open('src/pages/Utilisateurs.jsx', 'w', encoding='utf-8') as f:
    f.write(code)
print('OK - Utilisateurs.jsx cree!')